    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`p-5 rounded-2xl border shadow-lg flex items-center justify-between transition-colors ${
          ...
          <div>
            <div className={`text-xs uppercase tracking-wider font-medium ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>Rate Limit Enforcer</div>
            <div className={`text-xl font-bold my-1 ${isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}`}>5 / min • 20 / day</div>
            <div className="text-xs text-[#059669] font-semibold">Strict Client Window Control</div>
          </div>
        ...
        <div className={`p-5 rounded-2xl border shadow-lg flex items-center justify-between transition-colors ${
          ...
          <div>
            <div className={`text-xs uppercase tracking-wider font-medium ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>Banned & Blocked Users</div>
            <div className={`text-2xl font-bold my-1 ${activeBannedCount > 0 ? 'text-rose-500' : isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}`}>
              {activeBannedCount} Users
            </div>
            <div className="text-xs text-rose-500 font-semibold">Graduated Banning (5m → Permanent)</div>
          </div>
          <div className={`p-3 rounded-xl ${isDark ? 'bg-rose-500/20 text-rose-400' : 'bg-rose-50 text-rose-600'}`}>
        ...
        <div className={`p-5 rounded-2xl border shadow-lg flex items-center justify-between transition-colors ${
          ...
          <div>
            <div className={`text-xs uppercase tracking-wider font-medium ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>Attacks Intercepted</div>
            <div className={`text-2xl font-bold my-1 ${isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}`}>{totalAttackCount} Attempts</div>
            <div className="text-xs text-[#059669] font-semibold">Logged to security_attack_logs</div>
          </div>
          <div className={`p-3 rounded-xl ${isDark ? 'bg-[#2E6B5E]/30 text-[#34D399]' : 'bg-emerald-50 text-emerald-600'}`}>
        ...
        <div className={`p-5 rounded-2xl border shadow-lg flex items-center justify-between transition-colors ${
          ...
          <div>
            <div className={`text-xs uppercase tracking-wider font-medium ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>Institutional Guardrail</div>
            <div className={`text-2xl font-bold my-1 ${isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}`}>100% Active</div>
            <div className="text-xs text-[#059669] font-semibold">Zero-Bypass Code Boundary</div>
          </div>
          <div className={`p-3 rounded-xl ${isDark ? 'bg-[#1a2434] text-[#E1EED7]' : 'bg-slate-100 text-slate-700'}`}>
        ...
        <div className={`p-6 border-b flex justify-between items-center ${
          ...
          <button 
            ...
            className={`p-2 rounded-xl border transition-all flex items-center gap-1.5 text-xs font-semibold ${
              isDark ? 'bg-[#1a2434] hover:bg-[#243247] text-[#D0E7E1] border-[#243247]' : 'bg-slate-100 hover:bg-slate-200 text-slate-700 border-slate-200'
            }`}
          >
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className={`divide-y ${isDark ? 'divide-[#243247]/60' : 'divide-slate-100'}`}>
              {bannedUsers.length === 0 ? (
                <tr>
                  <td colSpan={6} className={`px-6 py-10 text-center ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>
                    No active banned or suspended users. All security guardrails are clear.
                  </td>
                </tr>
              ) : (
                bannedUsers.map((b: any) => (
                  <tr key={b.user_identifier} className={isDark ? 'hover:bg-[#1a2434]/60' : 'hover:bg-slate-50'}>
                    <td className="px-6 py-4 font-mono font-bold">
                      <div className={isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}>{b.user_identifier}</div>
                      <div className={`text-[11px] font-normal ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>IP: {b.user_ip || '127.0.0.1'}</div>
                    </td>
                    ...
                    <td className={`px-6 py-4 italic text-xs max-w-xs truncate ${isDark ? 'text-[#D0E7E1]' : 'text-slate-600'}`}>
                      "{b.reason || 'Security attack attempt'}"
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>
                      {b.last_offense_at ? new Date(b.last_offense_at).toLocaleString() : 'Recent'}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => handleUnban(b.user_identifier)}
                        className="px-3 py-1 rounded-lg bg-emerald-500/20 text-emerald-600 border border-emerald-500/30 hover:bg-emerald-500 hover:text-white transition-all text-xs font-semibold flex items-center gap-1 ml-auto"
                      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className={`divide-y ${isDark ? 'divide-[#243247]/60' : 'divide-slate-100'}`}>
              {attackLogs.length === 0 ? (
                <tr>
                  <td colSpan={5} className={`px-6 py-10 text-center ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>
                    No attack logs recorded in database. All security systems operating normally.
                  </td>
                </tr>
              ) : (
                attackLogs.map((log: any) => (
                  <tr key={log.log_id} className={isDark ? 'hover:bg-[#1a2434]/60' : 'hover:bg-slate-50'}>
                    <td className={`px-6 py-4 whitespace-nowrap font-mono ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>
                      {log.created_at ? new Date(log.created_at).toLocaleString() : 'Recent'}
                    </td>
                    <td className="px-6 py-4 font-mono font-semibold">
                      <div className={isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}>{log.user_identifier}</div>
                      <div className="text-[10px] text-slate-400">IP: {log.user_ip || '127.0.0.1'}</div>
                    </td>
                    <td className="px-6 py-4 font-mono">
                      <span className="px-2.5 py-1 rounded bg-rose-500/20 text-rose-500 border border-rose-500/30 font-bold text-[11px] inline-flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> {log.attack_type}
                      </span>
                    </td>
                    <td className={`px-6 py-4 italic text-xs max-w-sm truncate ${isDark ? 'text-[#D0E7E1]' : 'text-slate-700'}`}>
                      "{log.user_query}"
                    </td>
      ...
      <div className={`p-6 rounded-2xl border shadow-xl space-y-4 transition-colors ${
        ...
        <h2 className={`text-lg font-bold ${isDark ? 'text-[#F7F6ED]' : 'text-slate-900'}`}>Interactive Security Interceptor Policy Evaluator</h2>
        <p className={`text-xs ${isDark ? 'text-[#94a3b8]' : 'text-slate-500'}`}>Test prompt injection interceptors, instruction overrides, or rate-limiting responses in real-time.</p>
        ...
        <div className="space-y-3">
          <textarea
            ...
            className={`w-full p-3 border rounded-xl text-xs focus:outline-none transition-all ${
              isDark 
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#2E6B5E] text-white text-xs font-semibold hover:bg-[#34D399] hover:text-[#0b0f17] transition-all shadow-md"
          >
