...
export function TokenCostPanel({ metrics }: TokenCostPanelProps) {
  ...
  return (
    <div className="w-full bg-transparent text-ink mt-2 mb-1 p-2 border-l-2 border-[#D0CCE5]/50 pl-3">
      <div className="flex flex-col md:flex-row md:items-start gap-4 lg:gap-8">
        ...
        <div className="flex flex-col gap-2 shrink-0 md:w-[260px]">
          <div className="flex items-center gap-2">
            <div className="size-6 rounded-lg bg-gradient-to-br from-[#D0CCE5] to-[#D0E7E1] flex items-center justify-center text-[#2E6B5E] shadow-sm shrink-0">
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-2 mt-1">
             <div>
                <div className="text-[9px] uppercase font-bold text-ink-3 tracking-wider">Total Tokens</div>
                <div className="text-[13px] font-bold text-ink font-mono mt-0.5">{metrics.total_tokens.toLocaleString()}</div>
             </div>
             <div>
                <div className="text-[9px] uppercase font-bold text-ink-3 tracking-wider">Total Cost</div>
                <div className="text-[13px] font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">
                  {formattedCostUsd} <span className="text-[10px] font-normal text-ink-3">({formattedCostInr})</span>
                </div>
             </div>
             <div>
                <div className="text-[9px] uppercase font-bold text-ink-3 tracking-wider">Gen Speed</div>
                <div className="text-[12px] font-semibold text-ink mt-0.5">{metrics.tokens_per_sec} <span className="text-[10px] text-ink-3 font-normal">tok/s</span></div>
             </div>
             <div>
                <div className="text-[9px] uppercase font-bold text-ink-3 tracking-wider">Latency</div>
                <div className="text-[12px] font-semibold text-ink font-mono mt-0.5">{metrics.latency_ms}ms</div>
             </div>
          </div>
        </div>
        ...
        <div className="flex-1">
          <div className="text-[9px] uppercase font-bold text-ink-3 tracking-wider mb-2">Execution Pipeline</div>
          <div className="flex flex-wrap gap-2">
            {metrics.steps.map((step) => (
              <Tooltip key={step.step_number} content={step.details || step.model_name} position="top">
                <div className="flex items-center gap-1.5 rounded-full bg-transparent border border-line/60 px-2.5 py-1 text-[10px]">
                  <span className="font-bold text-accent shrink-0">{step.step_number}</span>
                  <span className="font-medium text-ink">{step.step_name}</span>
                  <span className="text-ink-3/70 font-mono text-[9px] pl-1 border-l border-line/60">
